...
module.exports = class SQLiteDatabase {
    ...
    insert(key, value) {
        ...
        if (this.exists(pattern)) {
            query = 'UPDATE data SET data = (?) WHERE key = (?)';
            set(data, key.match(/([^\.\[\]]+|\[\d+\])/g).slice(1), value);
        } else {
            query = 'INSERT INTO data (key, data) VALUES (?, ?)';
            data = value;
        }
        
        this.conn.prepare(query).run(key, data);
    ...
    delete(key) {
        ...
        this.conn.prepare('DELETE FROM data WHERE key=(?)').run(key);
