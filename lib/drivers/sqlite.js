...
const isValidPropertyValue = require('../../util/isValidPropertyValue');
...
module.exports = class SQLiteDatabase {
    ...
    insert(key, value) {
        ...
        const output = get(data, key);
        this.conn.prepare(query).run(pattern, typeof output === 'string' ? output : JSON.stringify(output));
    ...
    delete(key) {
        ...
        if (has(data, pattern)) this.insert(pattern, get(data, key));
    ...
    deleteMany() {
        ...
        return arr.filter((a) => this.toArray().find((b) => a === b.key));
    }
