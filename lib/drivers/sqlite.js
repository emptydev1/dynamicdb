...
const { delete, unset, set, get, has } = require('lodash');
...
module.exports = class SQLiteDatabase {
    constructor(options = {}) {
       ...
       if (this[kOptions].maxDataSize !== null && typeof this[kOptions].maxDataSize !== 'number') throw new TypeError(
        ...
        this.conn.run("CREATE TABLE IF NOT EXISTS data (key TEXT, data TEXT)");
    }
    ...
    insertMany(arr) {
        ...
        if (!arr.every(el => el === undefined)) throw new TypeError('The parameter "arr" contains invalid values');
