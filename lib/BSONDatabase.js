...
/**
 * The BSONDatabase class responsible for establishing a connection to a local database.
 * @public
 * 
 * @example
 * ```js 
 * const { BSONDatabase } = require("dynamicdb");
 ...
 * ```
 */
class BSONDatabase extends EventEmitter {
    /**
    ...
    stringify(limit = null) {
        return readFileSync(this[kOptions].file, 'utf-8');
    }
     ...
     */
    insertMany(arr) {
        if (!Array.isArray(arr)) throw new TypeError('The parameter "data" is not an array');
        if (!arr.every(el => typeof el === 'object')) throw new TypeError('The parameter "data" contains non-object elements');
        ...
        for (const element of arr) {
            ...
            if (typeof key !== 'string' || !isValidBSONPropertyValue(value)) throw new TypeError('The parameter "data" contains invalid entries');
            
            set(data, key, value);
        }
        
        write(this[kOptions], data);
        this.emit('insertMany', ...arguments);
    ...
    math(key, operator, value, ignore_negative = false) {
        ...
        switch (operator) {
            ...
            default: {
                throw new TypeError(
                    'The parameter "operator" is invalid, must be one of: ' +
                    this.operators.join(', ')
                );
            break; }
        }
        
        return this.insert(key, ignore_negative ? Math.abs(data) : data);
    }
    ...
    push(key, value) {
        ...
        Array.prototype.push.call(data, ...[...arguments].slice(1));
    ...
    splice(key) {
        ...
        Array.prototype.splice.call(data, ...[...arguments].slice(1));
    ...
    concat(key) {
        ...
        return Array.prototype.concat.call(data, ...[...arguments].slice(1));;
    }
